...
//         <div className="circle"></div>
//       </div>
//       <p className="text-gray-700 text-xl font-semibold mb-1">Loading your profile…</p>
//       <p className="text-gray-500 text-base">Please wait while we fetch your details</p>
//     </div>
//   );
...
function normalizeProfile(user = {}) {
  ...
  if (!Array.isArray(u.education) || u.education.length === 0) {
    u.education = [{ level: "", field: "" }];
  } else {
    u.education = u.education.map((e) => ({
      level: e?.level ?? "",
      field: e?.field ?? "",
    }));
  }

  if (typeof u.phone !== "string") u.phone = String(u.phone || "");

  if (u.dob) {
    try {
      const d = new Date(u.dob);
      if (!isNaN(d.getTime())) u.dob = d.toISOString().slice(0, 10);
    } catch {
      /* ignore */
    }
  }

  // If DB interest isn’t in our list, surface it via "Other" + interestOther
  if (u.interest && !interestFields.includes(u.interest)) {
    u.interestOther = u.interest;
    u.interest = "Other";
  } else {
    u.interestOther = "";
  }
...
export default function UserProfile() {
  ...
  // Guard session while on page (optional)
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch("/api/auth/me", { credentials: "include" });
  ...
  const onSubmit = async (form) => {
    try {
      const payload = {
        ...form,
        education: Array.isArray(form.education) ? form.education : [{ level: "", field: "" }],
        interest: form.interest === "Other" ? form.interestOther || "" : form.interest,
      };
      delete payload.interestOther;
      ...
      if (res.ok) {
        ...
        const normalized = normalizeProfile(updated.user || payload);
        setSavedData(normalized);
        reset(normalized, { keepDirty: false });
        setEditMode(false);
      } else {
        const msg = await res.text().catch(() => "");
        alert(`Failed to update profile${msg ? `: ${msg}` : ""}`);
      }
    } catch (err) {
      alert("Error: " + err.message);
    }
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="flex items-start justify-between">
          ...
          {!editMode && (
            <button
              ...
              className="h-10 px-6 rounded-lg bg-[#4F7BFF] text-white font-medium shadow-sm hover:brightness-105 transition"
            >
              Edit
            </button>
          )}
        ...
        <div className="mt-8">
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* User Name (required by API as `username`) */}
              <div>
                <label className="block text-sm text-gray-600 mb-2">User Name</label>
                <input
                  type="text"
                  ...
                  disabled={!editMode}
                  className={[inputShell, errors.username ? errBorder : okBorder, !editMode ? "opacity-100 cursor-not-allowed" : ""].join(" ")}
                />
                {errors.username && <p className="text-xs text-red-500 mt-1">Required field</p>}
              </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex gap-3" key={item.id}>
                      ...
                      {editMode && fields.length > 1 && (
                        <button
                          type="button"
                          onClick={() => remove(idx)}
                          className="sm:col-span-1 h-12 rounded-lg bg-red-50 text-red-600 font-bold"
                          title="Remove"
                        >
                          −
                        </button>
                  ...
                  {editMode && (
                    <button
                      ...
                      className="px-4 py-2 rounded-lg bg-[#4F7BFF] text-white text-sm font-medium"
                    >
                      + Add Qualification
                    </button>
                  )}
            ...
            {editMode && (
              <div className="flex justify-end gap-3 pt-8">
                ...
                <button
                  ...
                  className="h-10 px-6 rounded-lg bg-[#4F7BFF] text-white disabled:opacity-50"
                >
                  Save
                </button>
              </div>
        ...
        <div className="mt-12">
          ...
          <button
            type="button"
            disabled
            className="mt-5 px-4 py-2 rounded-lg bg-[#EFF3FF] text-[#4F7BFF] font-medium cursor-default"
          >
            + Add Email Address
          </button>
        </div>
