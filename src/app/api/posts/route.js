...
export async function GET(req) {
  try {
    ...
    const func = (searchParams.get("function") || "").trim();
    ...
    if (search) {
      ...
      baseMatch.$or = [
        ...
        { Skills: { $regex: regex } },
        { FunctionalArea: { $regex: regex } },
        { ["Functional Area"]: { $regex: regex } },
        { ExtractedRole: { $regex: regex } },
      ];
    }

    const hasFacet = !!industry || !!func || !!company || !!role;
     if (category && !hasFacet) {
    baseMatch.ExtractedRole = { $regex: `\\b${escapeRegex(category)}\\b`, $options: "i" };
  }
    if (type) {
      baseMatch["Job Type"] = { $regex: `^${escapeRegex(type)}$`, $options: "i" };
    }
    if (city) {
      baseMatch.City = { $regex: `^${escapeRegex(city)}$`, $options: "i" };
    }
    if (experience) {
      const mapped = experienceMapping[experience];
      if (mapped?.length) baseMatch["Experience Range"] = { $in: mapped };
    ...
    // ✅ Apply 4 facets
    if (industry) {
      baseMatch.Industry = { $regex: `^${escapeRegex(industry)}$`, $options: "i" };
    }
    if (func) {
      const escapedRaw = escapeRegex(func);
      const flexible = escapedRaw
        .replace(/\\\//g, "\\s*\\/\\s*") // allow spaces around "/"
        .replace(/\s+/g, "\\s+"); // collapse spaces to \s+

      const pattern = `^${flexible}$`;

      baseMatch.$and = (baseMatch.$and || []).concat({
        $or: [
          { FunctionalArea: { $regex: pattern, $options: "i" } },
          { ["Functional Area"]: { $regex: pattern, $options: "i" } },
        ],
      });
    }
    if (company) {
      baseMatch.Company = { $regex: `^${escapeRegex(company)}$`, $options: "i" };
    }
    if (role) {
      baseMatch.ExtractedRole = { $regex: `\\b${escapeRegex(role)}\\b`, $options: "i" };
    }
