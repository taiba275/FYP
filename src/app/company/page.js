    ...
    <div className="px-4 py-10 max-w-screen-xl mx-auto text-gray-800">
      {loading ? (
        <div className="w-full h-[50vh] flex flex-col justify-center items-center bg-white">
          ...
          <p className="text-gray-700 text-xl font-semibold mb-1">Loading Companies…</p>
          <p className="text-gray-500 text-base">Please wait while we organize the list</p>
        </div>
      ) : (
        <>
            ...
            <div key={L} id={L} className="mb-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {map[L].map(title => (
                  <Link
                    key={title}
                    href={`/?company=${encodeURIComponent(title)}&page=1&clear=1`}
